...
OpenCode.ai 是一个类似于 Claude Code 和 Codex 的编码代理。之前将 superpowers 移植到 OpenCode 的尝试（PR #93、PR #116）使用了文件复制方法。本设计采用不同的方法：使用 OpenCode 的 JavaScript/TypeScript 插件系统构建原生 OpenCode 插件，同时与 Codex 实现共享代码。
...
- **主代理**：Build（默认，完全访问权限）和 Plan（受限，只读）
- **子代理**：General（研究、搜索、多步骤任务）
- **调用方式**：主代理自动分派或通过 `@mention` 语法手动调用
...
3. **技能目录**
   - 核心：`~/.config/opencode/superpowers/skills/`（或已安装位置）
...
1. 创建 `lib/skills-core.js`
   - 从 `.codex/superpowers-codex` 提取前置元数据解析
   - 提取技能发现逻辑
   - 提取路径解析（含覆盖逻辑）
...
- **代码复用**：技能发现/解析的单一事实来源
- **可维护性**：漏洞修复同时适用于两个平台
- **可扩展性**：便于添加未来平台（Cursor、Windsurf 等）
