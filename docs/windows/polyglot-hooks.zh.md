...
Claude Code 插件需要在 Windows、macOS 和 Linux 上都能工作的钩子。本文档介绍实现这一目标的多语言包装器技术。
...
多语言脚本在多种语言中同时具有有效语法。我们的包装器在 CMD 和 bash 中都有效：
...
实际的钩子逻辑放在 `.sh` 文件中。为确保在 Windows 上（通过 Git Bash）正常工作：
...
对于拥有多个钩子的插件，您可以创建一个通用包装器，将脚本名称作为参数传入：
...
### "bash is not recognized"（bash 未被识别）
CMD 找不到 bash。包装器使用完整路径 `C:\Program Files\Git\bin\bash.exe`。如果 Git 安装在其他位置，请更新路径。

### "cygpath: command not found" 或 "dirname: command not found"（找不到命令）
Bash 未以登录 shell 运行。确保使用了 `-l` 标志。
